...
Traces, logs, and metrics flush sequentially. If all three exporters are enabled and the Collector is unreachable, one request can initially spend up to roughly three export timeouts before each signal's circuit breaker opens. Keep the default `200ms`, use a local Collector, and never copy backend-scale timeouts such as `10s` into a synchronous PHP-FPM exporter.
